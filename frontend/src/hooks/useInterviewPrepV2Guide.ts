...
export function useInterviewPrepV2Guide() {
  const {
    ...
    setIsGeneratingInterviewPrepGuide
  ...
  const queryResult = useQuery<InterviewPrepV2Guide, Error>({
    ...
    // keepPreviousData: true, // Prevent data from becoming undefined when query is disabled after success -- Temporarily commented out due to persistent TS error
    queryFn: async () => {
      ...
      console.log("useInterviewPrepV2Guide: Raw section_1_company_industry data:", JSON.stringify(generatedGuide?.section_1_company_industry, null, 2));
  ...
  useEffect(() => {
    if (queryResult.isSuccess || queryResult.isError) {
      console.log('useInterviewPrepV2Guide: Query settled via useEffect. Resetting generation flags.');
      if (setIsGeneratingInterviewPrepGuide) {
        setIsGeneratingInterviewPrepGuide(false);
      }
      if (setShouldGenerateGuide) {
        setShouldGenerateGuide(false);
      }
    }
  }, [queryResult.isSuccess, queryResult.isError, setIsGeneratingInterviewPrepGuide, setShouldGenerateGuide]);
